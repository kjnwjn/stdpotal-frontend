...
export default {
    ...
    methods: {
        ...
        async newDish() {
            ...
            await axios
                .post(`${process.env.VUE_APP_API_GATEWAY}/dish/new-dish?token=${this.accessToken}`, formData, { headers })
                .then((res) => {
                    console.log(res);
                    if (res.data.status) {
        ...
        async dishRemove(dishId) {
            this.isLoading = true;
            await axios.delete(`${process.env.VUE_APP_API_GATEWAY}/dish/${dishId}?token=${this.accessToken}`).then((res) => {
                if (res.data.status) {
                    this.toastify.success(res.data.msg.en);
                } else {
                    this.toastify.error(res.data.msg.en);
        ...
        async switchStatus(dishId) {
            this.isLoading = true;
            await axios.put(`${process.env.VUE_APP_API_GATEWAY}/dish/update-dish-status?dishId=${dishId}&token=${this.accessToken}`).then((res) => {
                if (res.data.status) {
                    this.toastify.success(res.data.msg.en);
                } else {
                    this.toastify.error(res.data.msg.en);
        ...
        async fetchData() {
            this.isLoading = true;
            await axios
                .get(`${process.env.VUE_APP_API_GATEWAY}/dish/get-all?token=${this.accessToken}`)
                .then((res) => {
