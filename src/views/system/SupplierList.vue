...
export default {
    ...
    methods: {
        ...
        async fetchData(page = 1) {
            ...
            await axios
                .get(`${process.env.VUE_APP_API_GATEWAY}/supplier/get-all?page=${pageSelect}&token=${this.accessToken}`)
                .then((res) => {
        ...
        async handleDelete(barcode = null) {
            this.isLoading = true;
            await axios
                .delete(`${process.env.VUE_APP_API_GATEWAY}/supplier/delete/${encodeURIComponent(barcode)}?token=${this.accessToken}`)
                .then((res) => {
                    if (res.data.status) {
