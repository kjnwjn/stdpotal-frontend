...
export default {
    ...
    methods: {
        ...
        async attendanceHandler(type) {
            this.isLoading = true;
            await axios
                .post(`${process.env.VUE_APP_API_GATEWAY}/attendance/${type == "checkIn" ? "checkin" : "checkout"}?token=${this.accessToken}`, {
        ...
        async confirmAccountHandler() {
            this.isLoading = true;
            await axios
                .get(`${process.env.VUE_APP_API_GATEWAY}/account/${this.accountDetail.userCode ? this.accountDetail.userCode : "null"}/detail?token=${this.accessToken}`)
                .then((res) => {
                    if (res.data.status) {
        ...
        async getAttendanceHandler() {
            ...
            if (this.startDate && this.endDate) {
                ...
                await axios.get(`${process.env.VUE_APP_API_GATEWAY}/attendance/report?dateFrom=${dateFrom}&dateTo=${dateTo}&token=${this.accessToken}`).then((res) => {
                    if (res.data.status) {
