...
export default {
    ...
    methods: {
        ...
        async fetchData(page = 1) {
            ...
            await axios.get(`${process.env.VUE_APP_API_GATEWAY}/product/get-out-of-stock?page=${pageSelect}&token=${this.accessToken}`).then((res) => {
                if (res.data.status) {
        ...
        async exportHandler() {
            ...
            await axios({
                url: `${process.env.VUE_APP_API_GATEWAY}/product/out-of-stock/export?requestID=${requestID}&token=${this.accessToken}`,
                method: "GET",
                responseType: "blob",
            }).then((response) => {
