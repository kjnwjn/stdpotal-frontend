<template>
    <main>
        ...
        <div class="flex items-center mb-4 text-green-700 font-bold text-lg uppercase">
            ...
            <h1 class="ml-2">Employee salary</h1>
        </div>
                    ...
                    <table class="overflow-scroll w-full text-sm text-left text-gray-500 dark:text-gray-400 mb-4">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="py-3 px-6">Total Salary</th>
                            </tr>
                        </thead>
                        <tbody v-if="dataList.length > 0">
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" v-for="(employee, i) in dataList" :key="i">
                                ...
                                <td class="py-4 px-6">{{ employee.finalWorkTime }}</td>
                                <td class="py-4 px-6">{{ priceFormat(employee.tempSalary) }}</td>
                            </tr>
                        </tbody>
                    </table>
    ...
    methods: {
        ...
        async visualizeHandler() {
            if (this.startDate && this.endDate) {
                ...
                await axios.get(`${process.env.VUE_APP_API_GATEWAY}/employee/salary/visualize/?dateFrom=${dateFrom}&dateTo=${dateTo}&token=${this.accessToken}`).then((res) => {
                    if (res.data.status && res.data.result.length > 0) this.dataList = res.data.result;
                });
        ...
        async exportHandler() {
            ...
            const dateTo = `${dayTo}/${monthTo}/${yearTo}`;
            await axios({
                url: `${process.env.VUE_APP_API_GATEWAY}/employee/salary/visualize/export?dateFrom=${dateFrom}&dateTo=${dateTo}&token=${this.accessToken}`,
                method: "GET",
                responseType: "blob",
            }).then((response) => {
                ...
                const fileName = `employee-salary-reports-${dateFrom}-${dateTo}-${Date.now()}.xlsx`;
