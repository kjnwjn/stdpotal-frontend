...
export default {
    ...
    methods: {
        async updateSalaryHandler() {
            this.isLoading = true;
            await axios
                .post(`${process.env.VUE_APP_API_GATEWAY}/employee/salary/updateSalary?token=${this.accessToken}`, {
        ...
        async fetchData() {
            this.isLoading = true;
            await axios.get(`${process.env.VUE_APP_API_GATEWAY}/employee/${this.$route.params.userCode}/detail?token=${this.accessToken}`).then((res) => {
