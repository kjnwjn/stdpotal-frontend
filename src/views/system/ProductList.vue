...
export default {
    ...
    methods: {
        ...
        async fetchData(page = 1) {
            ...
            await axios.get(`${process.env.VUE_APP_API_GATEWAY}/product/get-all?page=${pageSelect}&token=${this.accessToken}`).then((res) => {
        ...
        async handleDelete(barcode = null) {
            ...
            console.log(barcode);
            await axios
                .delete(`${process.env.VUE_APP_API_GATEWAY}/product/delete/${barcode}?token=${this.accessToken}`)
                .then((res) => {
                    if (res.data.status) {
