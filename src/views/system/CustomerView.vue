...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            this.isLoading = true;
            await axios.get(`${process.env.VUE_APP_API_GATEWAY}/customer/get-all?token=${this.accessToken}`).then((res) => {
                if (res.data.status) {
        ...
        async newCustomerHandler() {
            this.isLoading = true;
            await axios
                .post(`${process.env.VUE_APP_API_GATEWAY}/customer/new?token=${this.accessToken}`, {
        ...
        async customerSearchingHandler() {
            await axios.get(`${process.env.VUE_APP_API_GATEWAY}/customer/detail?customerID=${this.customerIDToSearch}&token=${this.accessToken}`).then((res) => {
                if (res.data.status) {
                    this.toastify.success(res.data.msg.en);
                    this.customerList = [res.data.data];
                } else {
